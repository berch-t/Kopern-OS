#!/usr/bin/env bash
#
# install.sh -- Install Kopern-OS Catalog agents into your local agentic tool(s).
#
...
#   gemini-cli   -- Install extension to ~/.gemini/extensions/kopern-os-agents/
#   opencode     -- Copy agents to .opencode/agents/ in current directory
#   cursor       -- Copy rules to .cursor/rules/ in current directory
#   aider        -- Copy CONVENTIONS.md to current directory
#   windsurf     -- Copy .windsurfrules to current directory
#   openclaw     -- Copy workspaces to ~/.openclaw/kopern-os-agents/
...
tool_label() {
  case "$1" in
    ...
    openclaw)    printf "%-14s  %s" "OpenClaw"     "(~/.openclaw/kopern-os-agents)" ;;
...
interactive_select() {
  ...
  while true; do
    ...
    printf "\n"
    box_top
    box_row "${C_BOLD}  Kopern-OS Catalog -- Tool Installer${C_RESET}"
    box_bot
    printf "\n"
...
install_gemini_cli() {
  ...
  local dest="${HOME}/.gemini/extensions/kopern-os-agents"
  local count=0
...
install_openclaw() {
  ...
  local dest="${HOME}/.openclaw/kopern-os-agents"
  local count=0
...
main() {
  ...
  if $use_interactive; then
    ...
    header "Kopern-OS Catalog -- Scanning for installed tools..."
    printf "\n"
    local t
    for t in "${ALL_TOOLS[@]}"; do
      if is_detected "$t" 2>/dev/null; then
  ...
  printf "\n"
  header "Kopern-OS Catalog -- Installing agents"
  printf "  Repo:       %s\n" "$REPO_ROOT"
